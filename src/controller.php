...
class Controller
{
    ...
    public function searchPlaylists() {
        ...
        $search_text = filter_input(INPUT_GET, 'search_text', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
        ...
        $user_query = filter_input(INPUT_GET, 'user_query', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?: '';
        ...
        $results_sql = "
            SELECT DISTINCT p.id, p.name, u.username, p.created_at
            " . $base_sql . $where_sql . "
            ORDER BY p.created_at DESC
            LIMIT ? OFFSET ?
        ";
